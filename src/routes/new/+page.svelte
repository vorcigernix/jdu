...
{#if form?.success}
	<section class="py-6  text-zinc-50">
		...
		>
			<h1
				class="text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-amber-500 via-red-500 to-yellow-500 background-animate text-center"
			>
				Jop, máš to tam.
			</h1>
...
{:else}
	<section class="flex flex-wrap lg:h-screen pt-20 md:pt-0">
		<div class="w-full px-4 py-12 sm:px-6 sm:py-16 lg:w-1/2 lg:px-8">
			...
			<form
				method="POST"
				class="mx-auto mt-8 mb-0 max-w-md space-y-4 text-zinc-900 {submitting && `opacity-10`}"
			...
			>
				<div>
					...
					<div>
						<input
							...
							name="username"
							value={$userNameStore}
							class="w-full border-zinc-200 p-4 pr-12 text-sm shadow-sm"
				...
				<div>
					<fieldset>
						<legend class="text-sm text-zinc-400">Jak často</legend>
						<div class="flex py-2 font-bold space-x-2 flex-grow justify-between md:justify-start">
							<div class="">
								<input
									type="radio"
									id="frequencyChoice1"
								...
								<label
									...
									class="p-2 peer-checked:bg-amber-500 peer-checked:text-zinc-900 text-zinc-50 cursor-pointer"
									>Každej tejden</label
								>
							</div>
							<div class="">
						...
						<span class="text-xs text-zinc-400">*počínaje tímto neasi</span>
					</fieldset>
				</div>
				<div>
				...
				<div>
					...
					<textarea
						...
						class="w-full text-zinc-800 text-sm"
						required
					/>
				</div>
				...
				<div class="flex items-center justify-between">
					...
					<button class="ml-3 inline-block  bg-amber-500 px-5 py-3 font-bold text-zinc-900">
						Založit
					</button>
				</div>
			</form>
